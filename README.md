...
- клиенты: название, контактное лицо, телефон, эл. почта, адрес, юридические реквизиты, счета, договор, подписант, комментарий
...
- работы: номер заказа, клиент, исполнитель, тип работы, описание, врач, пациент, зубная формула, статус, даты, финансовые поля, материалы, технологические операции

## Печатные формы и реквизиты

В проекте добавлены печатные формы:

- `Счет`
- `Акт выполненных работ`
- `Наряд`

Что для этого уже реализовано:

- у клиента есть юридические реквизиты, банковские счета, номер и дата договора, подписант
- есть отдельный раздел `Организация`, где задаются реквизиты самой лаборатории
- из карточки работы можно открыть печатные HTML-версии `счета`, `акта` и `наряда`
- backend отдает документы через `/api/v1/documents/works/{work_id}/...`

Перед использованием печатных форм заполни:

- реквизиты клиента в разделе `Клиенты`
- реквизиты лаборатории в разделе `Организация`

## Вложения к работам

Для каждой работы можно прикреплять файлы:
...
- не более `2 МБ` на файл
- типы: `jpg`, `jpeg`, `png`, `webp`, `gif`, `pdf`, `doc`, `docx`, `xls`, `xlsx`

По умолчанию файлы хранятся локально в `storage/work_attachments`. Если перед лабораторией стоит отдельный CDN или файловый origin, можно задать:
