...
class FakeNarad:
    ...
    status_logs: list = field(default_factory=list)
...
class FakeWork:
    ...
    @property
    def narad_number(self) -> str:
        return self.narad.narad_number if self.narad else self.order_number
...
class FakeWorkRepository:
    ...
    async def add(self, work):
        ...
        if hasattr(self, "narad_repository"):
            narad = self.narad_repository.narads.get(fake.narad_id)
            fake.narad = narad
            if narad is not None:
                narad.works.append(fake)
...
class FakeNaradRepository:
    def __init__(self, client: FakeClient):
        self.client = client
        self.narads: dict[str, FakeNarad] = {}

    async def add(self, narad):
        fake = FakeNarad(
            ...
            require_color_photo=getattr(narad, "require_color_photo", False),
            face_shape=getattr(narad, "face_shape", None),
            implant_system=getattr(narad, "implant_system", None),
            metal_type=getattr(narad, "metal_type", None),
            shade_color=getattr(narad, "shade_color", None),
            tooth_formula=getattr(narad, "tooth_formula", None),
            tooth_selection=getattr(narad, "tooth_selection", None),
            status=narad.status,
        ...
        self.narads[fake.id] = fake
        return fake

    async def get(self, narad_id: str):
        return self.narads.get(narad_id)

    async def add_status_log(self, log):
        narad = self.narads[log.narad_id]
        ...
        narad.status_logs.insert(0, fake)
        return fake
...
class FakeMaterialRepository:
    ...
    async def change_stock(
        ...
        *,
        ...
        self.consumed.append((material_id, abs(quantity_delta)))
...
class FakeContextUoW:
    def __init__(self, *, client: FakeClient, executor: FakeExecutor, material: FakeMaterial):
        ...
        self.narads = FakeNaradRepository(client)
        ...
        self.works.narad_repository = self.narads
...
def register_test_narad(
    uow: FakeContextUoW,
    client: FakeClient,
    *,
    narad_number: str,
    title: str = "Тестовый наряд",
    received_at: datetime | None = None,
    **overrides,
) -> FakeNarad:
    narad = FakeNarad(
        narad_number=narad_number,
        client_id=client.id,
        title=title,
        status="new",
    ...
    uow.narads.narads[narad.id] = narad
    return narad
...
async def test_create_work_applies_client_adjustment_and_creates_change_log():
    ...
    catalog_item = SimpleNamespace(
        ...
        name="Коронка",
        category="Ортопедия",
        description="Базовая коронка",
        base_price=Decimal("10000.00"),
        default_duration_hours=Decimal("2.00"),
        default_operations=[],
    )
    uow.work_catalog.items[catalog_item.id] = catalog_item
    narad = register_test_narad(
        uow,
        client,
        narad_number="DL-TEST-001",
        title="Коронка",
    ...
    assert result.narad_number == "DL-TEST-001"
...
@pytest.mark.asyncio
async def test_create_work_in_existing_narad_keeps_narad_header_values():
    ...
    uow.work_catalog.items[catalog_item.id] = catalog_item
    existing_narad = FakeNarad(
        id=str(uuid4()),
        narad_number="NAR-2026-0001",
        client_id=client.id,
        title="Составной ортопедический заказ",
        status="new",
        received_at=datetime.now(timezone.utc),
        doctor_name="Сергей Волков",
        doctor_phone="+79991112233",
    ...
    uow.narads.narads[existing_narad.id] = existing_narad
    search = FakeSearchService()
    cache = FakeCacheService()
    service = WorkService(uow=uow, search=search, cache=cache)
    ...
    narad = uow.narads.narads[existing_narad.id]

    assert result.narad_id == existing_narad.id
    assert result.order_number == "NAR-2026-0001"
    assert narad.title == "Составной ортопедический заказ"
    assert narad.patient_name == "Ирина Соколова"
    assert narad.doctor_name == "Сергей Волков"
    assert len(narad.works) == 1
    assert narad.status == "new"
    assert search.calls
    assert cache.invalidated == ["dashboard:"]


@pytest.mark.asyncio
async def test_create_work_requires_catalog_item():
    client = FakeClient(id=str(uuid4()), name="Smile Studio", default_price_adjustment_percent=Decimal("0.00"))
    executor = FakeExecutor(id=str(uuid4()), full_name="Dmitry Ivanov", hourly_rate=Decimal("2500.00"))
    material = FakeMaterial(id=str(uuid4()), name="Zircon Disc", average_price=Decimal("3000.00"))
    uow = FakeContextUoW(client=client, executor=executor, material=material)
    narad = register_test_narad(uow, client, narad_number="DL-TEST-001A")
...
async def test_create_work_uses_operation_catalog_rate_for_executor_category():
    ...
    uow.work_catalog.items[catalog_item.id] = catalog_item
    narad = register_test_narad(uow, client, narad_number="DL-TEST-002")
    service = WorkService(uow=uow, search=FakeSearchService(), cache=FakeCacheService())
...
async def test_create_work_uses_client_specific_catalog_price_without_default_adjustment():
    ...
    uow = FakeContextUoW(client=client, executor=executor, material=material)
    uow.work_catalog.items[catalog_item.id] = catalog_item
    narad = register_test_narad(uow, client, narad_number="DL-TEST-003")
    service = WorkService(uow=uow, search=FakeSearchService(), cache=FakeCacheService())
...
async def test_create_work_builds_multiple_work_items_and_aggregates_total():
    ...
    narad = register_test_narad(uow, client, narad_number="DL-TEST-004")
    service = WorkService(uow=uow, search=FakeSearchService(), cache=FakeCacheService())
...
async def test_update_status_does_not_close_work_automatically():
    ...
    uow.work_catalog.items[catalog_item.id] = catalog_item
    narad = register_test_narad(uow, client, narad_number="DL-TEST-005")
    service = WorkService(uow=uow, search=FakeSearchService(), cache=FakeCacheService())
...
async def test_close_and_reopen_work_manage_closed_at_separately():
    ...
    uow.work_catalog.items[catalog_item.id] = catalog_item
    narad = register_test_narad(uow, client, narad_number="DL-TEST-006")
    service = WorkService(uow=uow, search=FakeSearchService(), cache=FakeCacheService())
